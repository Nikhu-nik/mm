...
export class PostadPage implements OnInit {
 ...
 advimgbase;
  ...
  PickFile(){
    ...
    this.advimage.push(this.base64temp);
 ...
 //this.filePath.resolveNativePath(val).then((nativepath)=>{
  ...
  uploadmyadd() {
    ...
    for(var k = 0;k<this.advimage.length;k++)
    {
    ...
    for(var i = 0;i<this.advimage.length;i++)
    {
    ...
    console.log(this.advimage);
    this.data["advimgbase"] = this.advimage[i];
...
PickfromGallery(){
  ...
  this.picker.getPictures(options).then((arr) => {
    for (var i = 0; i < arr.length; i++) {
      this.advimage.push("data:image/png;base64," + arr[i]);
    }
